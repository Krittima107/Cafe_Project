...
$stmt5 = $conn->query("SELECT COUNT(*) FROM orders");
$total_orders = $stmt5->fetchColumn();

$stmt6 = $conn->query("SELECT SUM(total_price) FROM orders WHERE status='completed'");
$total_revenue = $stmt6->fetchColumn();
if(!$total_revenue){
$total_revenue = 0;
}
?>
...
<div class="header">
<h2 style="margin:0;color:var(--color-brown-dark);">
⚙️ ระบบจัดการร้าน (Dashboard)
</h2>
</div>
...
<div class="stat-card" style="background:#e8f5e9;border-color:var(--color-green);">
<h3>หมวดหมู่สินค้า</h3>
<div class="number"><?php echo $total_categories; ?></div>
<span style="font-size:14px;">หมวดหมู่</span>
</div>

<div class="stat-card" style="background:#fff3e0;border-color:orange;">
<h3>เมนูที่สินค้าหมด</h3>
<div class="number" style="color:orange;"><?php echo $out_of_stock; ?></div>
<span style="font-size:14px;">รายการ</span>
</div>

<div class="stat-card" style="background:#f3e5f5;border-color:purple;">
<h3>ผู้ใช้งานในระบบ</h3>
<div class="number" style="color:purple;"><?php echo $total_users; ?></div>
<span style="font-size:14px;">บัญชี</span>
</div>

<div class="stat-card" style="background:#e3f2fd;border-color:#2196f3;">
<h3>คำสั่งซื้อทั้งหมด</h3>
<div class="number" style="color:#2196f3;"><?php echo $total_orders; ?></div>
<span style="font-size:14px;">ออเดอร์</span>
</div>

<div class="stat-card" style="background:#e8f5e9;border-color:#4caf50;">
<h3>รายได้รวม</h3>
...
<a href="manage_menu.php" class="btn btn-green">
☕ จัดการเมนู
</a>

<a href="orders.php" class="btn btn-green" style="background:#ff9800;">
📦 ดูคำสั่งซื้อ
</a>

<a href="../index1.php" class="btn btn-brown">
🏠 ดูหน้าร้าน (Frontend)
</a>
