        ...
        <div class="container">
            ...
            <div style="background: var(--color-cream); padding: 15px; border-radius: 8px; margin-bottom: 20px;">
                ...
                <form action="manage_menu.php" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <label style="font-weight: normal; cursor: pointer;"><input type="checkbox" name="is_available"
                                checked> เมนูนี้พร้อมขาย</label>
                    </div>
                ...
                <tbody>
                    <?php foreach ($menus as $menu): ?>
                        <tr>
                            ...
                            <td><span
                                    style="background: white; padding: 2px 8px; border-radius: 12px; font-size: 14px; border: 1px solid var(--color-brown-light);"><?php echo $menu['description']; ?></span>
                            </td>
                            <td><?php echo number_format($menu['price'], 2); ?> ฿</td>
                            <td><?php echo $menu['is_available'] ? '<span style="color:green;">พร้อมขาย</span>' : '<span style="color:red;">หมด</span>'; ?>
                            </td>
                            <td>
                                <a href="edit_menu.php?id=<?php echo $menu['id']; ?>" class="btn btn-brown"
                                    style="padding: 4px 8px; font-size: 14px;">แก้ไข</a>

                                <a href="manage_menu.php?delete_id=<?php echo $menu['id']; ?>" class="btn"
                                    style="background: red; padding: 4px 8px; font-size: 14px;"
                                    onclick="return confirm('แน่ใจหรือไม่ว่าต้องการลบเมนูนี้?');">ลบ</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                    <?php if (count($menus) == 0): ?>
                        <tr>
                            <td colspan="7" style="text-align:center;">ยังไม่มีรายการเมนู</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
