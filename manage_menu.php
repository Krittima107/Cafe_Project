...
// --- ส่วนจัดการเมื่อมีการกดปุ่ม "เพิ่มเมนู" (Create) ---
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_menu'])) {
    ...
    if (isset($_FILES['menu_image']) && $_FILES['menu_image']['error'] == 0) {
        ...
        // Validation: เช็คชนิดและขนาดไฟล์ (ไม่เกิน 2MB)
        if (in_array($fileType, $allowedTypes) && $fileSize <= 2 * 1024 * 1024) {
            $fileExtension = pathinfo($fileName, PATHINFO_EXTENSION);
            $newFileName = "menu_" . uniqid() . "." . $fileExtension; // เปลี่ยนชื่อกันซ้ำ
            $destPath = "../uploads/menus/" . $newFileName;
    ...
    // บันทึกลงฐานข้อมูล (ถ้าไม่มี Error การอัปโหลด)
    if (empty($msg)) {
        try {
            ...
            $msg = "<div style='color:green;'>✅ เพิ่มเมนูสำเร็จ!</div>";
...
// --- ส่วนดึงข้อมูลเพื่อมาแสดงผล (Read) ---
// ดึงหมวดหมู่มาทำ Dropdown
$cat_stmt = $conn->query("SELECT * FROM categories");
$categories = $cat_stmt->fetchAll(PDO::FETCH_ASSOC);

// ดึงเมนูทั้งหมดมาแสดงในตาราง (JOIN กับ categories เพื่อเอาชื่อหมวดหมู่)
    ...
    <div class="container">
        ...
        <div style="background: var(--color-cream); padding: 15px; border-radius: 8px; margin-bottom: 20px;">
            ...
            <form action="manage_menu.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label><input type="checkbox" name="is_available" checked> พร้อมขาย</label>
                </div>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <a href="#" class="btn btn-brown" style="padding: 4px 8px; font-size: 14px;">แก้ไข</a>
                        <a href="#" class="btn" style="background: red; padding: 4px 8px; font-size: 14px;">ลบ</a>
                    </td>
                </tr>
                <?php endforeach; ?>
                <?php if(count($menus) == 0): ?>
                    <tr><td colspan="6" style="text-align:center;">ยังไม่มีรายการเมนู</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
